...
export const useApiQuery = <T>({
  ...
  return useQuery({
    queryKey,
    queryFn: async () => {
      console.log(`API Query: ${queryKey.join('/')}`);
      try {
        const result = await queryFn();
        console.log(`API Query Success: ${queryKey.join('/')}`);
        onSuccess?.(result);
        return result;
      } catch (error) {
...
export const useApiMutation = <TData, TVariables>({
  ...
  return useMutation({
    mutationFn: async (variables: TVariables) => {
      console.log('API Mutation:', variables);
      
      try {
        const result = await mutationFn(variables);
        console.log('API Mutation Success:', result);
        return result;
      } catch (error) {
